...
static unsigned int __stdcall chuni_io_led_pipe_thread_proc(void *ctx)
{
    ...
    while (true)
    {
        ...
        while (fConnected)
        {
            ...
            {
				OutputDebugStringA("ChuniVR: LED pipe mutex wait failed\n");
                continue;
            }
            ...
            for (int i = 0; i < LED_BOARDS_TOTAL; i++) {
                if (pipe_update[i])
                {
                    ...
                    if (result != S_OK)
                    {
						OutputDebugStringA("ChuniVR: LED pipe write failed\n");
                        //if (result == E_ABORT) 
                        //{
                            fConnected = false;
                        //}
...
HRESULT led_pipe_init()
{
    ...
    {
		OutputDebugStringA("ChuniVR: Failed to create LED pipe mutex\n");
        return E_FAIL;
    }
    ...
    _beginthreadex(
            ...
            0,
            NULL);
	OutputDebugStringA("ChuniVR: LED pipe thread started\n");
    return S_OK;
}
